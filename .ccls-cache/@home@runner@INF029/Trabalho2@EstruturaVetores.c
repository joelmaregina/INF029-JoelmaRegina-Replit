...
{
    int retorno = validaPosicao(posicao);
    ...
    if (vetorPrincipal[posicao - 1].aux != NULL ) return JA_TEM_ESTRUTURA_AUXILIAR;
    else {
      vetorPrincipal[posicao - 1].aux = malloc(sizeof(estrutura) * tamanho);
...
void inicializar()
{
  for (int i = 0; i < TAM; i++) vetorPrincipal[i].aux = NULL;
}
